...
- [ ] **T-FE-003**: Configurar `devDependencies`: `typescript ^5.x`, `openapi-typescript ^7.x`, `eslint ^8.57.0`, `eslint-config-expo`, `prettier ^3.x`
...
- [ ] **T-FE-013**: ~~Implementar `merge-sort.ts`~~ — **DIFERIDO**: No existe engine backend ni seed para Merge Sort. Implementar cuando se agregue el engine `merge-sort.engine.ts` en el backend
...
- [ ] **T-FE-088**: Crear `AppNavigator.tsx` — Navigator raíz que evalúa estado y "tipo" del usuario para decidir stack: `AuthNavigator` (no autenticado), `MainTabNavigator` (tipo==="usuario"), o `AdminNavigator` (tipo==="administrador"). (Ref: `admin-access-routing.spec.md`)
